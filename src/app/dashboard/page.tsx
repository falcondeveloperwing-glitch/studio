...
export default function DashboardOverview() {
  ...
  const liveFeedQuery = useMemo(() => {
    if (!db || !user) return null;
    return query(collection(db, 'users', user.uid, 'live_feed'), orderBy('timestamp', 'desc'), limit(10));
  }, [db, user]);

  const { data: liveFeed, loading: feedLoading } = useCollection(liveFeedQuery);

  return (
    <div className="p-8 lg:p-12 space-y-12 animate-in fade-in zoom-in-95 duration-1000">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="font-headline text-4xl font-bold mb-2 tracking-tight">System Operational, {user?.displayName || 'Partner'}</h1>
          <p className="text-muted-foreground text-lg">Your AI fleet is monitoring <span className="text-white font-medium">142</span> active shoppers.</p>
        </div>
        ...
        <GlassCard variant="darker" className="py-2 px-4 flex items-center gap-4 border-white/5">
          ...
          <div className="text-sm">
            <p className="font-bold text-white">42 Online</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Response Speed', value: '0.8s', change: '-20%', icon: Clock, color: 'accent' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <GlassCard className="lg:col-span-2 border-white/5" variant="darker">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h3 className="font-headline font-bold text-xl tracking-tight mb-1">Conversion Velocity</h3>
              <p className="text-xs text-muted-foreground tracking-wide uppercase font-bold">Revenue Recovery Over Time</p>
            </div>
          </div>
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <XAxis dataKey="name" stroke="rgba(255,255,255,0.1)" fontSize={12} tickLine={false} axisLine={false} dy={10} />
                <YAxis hide />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0a0a0c', border: '1px solid #222', borderRadius: '12px' }}
                  itemStyle={{ color: '#fff', fontSize: '12px', fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="revenue" stroke="hsl(var(--primary))" fillOpacity={1} fill="url(#colorRevenue)" strokeWidth={4} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </GlassCard>
        ...
        <GlassCard className="flex flex-col h-full border-white/5" variant="darker">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
            {feedLoading ? (
              ...
              liveFeed.map((item, i) => (
                <div key={item.id} className="flex gap-4 group cursor-pointer hover:translate-x-1 transition-transform duration-300">
              ...
              <p className="text-xs text-muted-foreground italic">System warm-up in progress...</p>
            )}
          </div>
            ...
            <GlassCard variant="borderless" className="p-4 bg-primary/5 border border-primary/10 group cursor-pointer">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-white">AI Optimization Advice</span>
                </div>
